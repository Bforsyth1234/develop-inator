...
async def _execute_approved_spec(container, execution_id: str) -> None:
    """Async helper that runs the approved spec via OpenHands or Aider."""
    ...
    selected_model = execution.model or container.action.aider_model_standard
    use_openhands = execution.executor == "openhands" and container.action.openhands_enabled

    try:
        if use_openhands:
            result = await container.action._run_openhands(
                req,
                optimized_prompt=execution.generated_spec,
                openhands_model=selected_model,
                target_repo_key=target_repo_key,
            )
        else:
            result = await container.action._run_aider(
                req,
                optimized_prompt=execution.generated_spec,
        ...
        if result.returncode != 0:
            if not use_openhands and result.test_attempts > 0:
                await container.action._handle_test_failure(req, result)
            else:
                await container.action._handle_failure(req, result)
        else:
            await container.action._handle_success(
                req, result, model=selected_model, target_repo_key=target_repo_key,
            )
    except Exception:
